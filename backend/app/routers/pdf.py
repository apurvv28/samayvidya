    ...
    try:
        ...
        if entity_id:
            if section == "division":
                ...
                room_lookup = {str(item.get("room_id")): str(item.get("room_name") or item.get("room_number") or item.get("room_id")) for item in rooms}
                scope_label = room_lookup.get(str(entity_id), str(entity_id))
            else:
                faculty_lookup = {str(item.get("faculty_id")): str(item.get("faculty_name") or item.get("faculty_id")) for item in faculty}
